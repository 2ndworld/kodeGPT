...
describe("service runtime readiness state", () => {
  ...
  it("reads prior surface 0.20 and current surface 0.21 readiness without dropping compatibility", async () => {
    const { store } = await storeFixture();
    const prior = { ...ready(), surfaceVersion: "0.20" as const };
    await writeFile(store.path, JSON.stringify(prior), "utf8");
    await expect(store.read()).resolves.toEqual(prior);

    const current = { ...ready(), surfaceVersion: "0.21" as const };
