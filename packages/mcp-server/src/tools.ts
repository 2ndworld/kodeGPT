...
export function registerKodegptTools(server: McpServer, context: KodegptToolContext): void {
  ...
  server.registerTool(
    "git.diff",
    {
      description: "Inspect the current workspace Git diff through the hardened read-only sandbox.",
      inputSchema: { workspaceId: z.string().min(1) },
      annotations: READ_ONLY_TOOL_ANNOTATIONS
    },
  ...
  server.registerTool(
    "git.status",
    {
      description: "Inspect the current workspace Git status through the hardened read-only sandbox.",
      inputSchema: { workspaceId: z.string().min(1) },
      annotations: READ_ONLY_TOOL_ANNOTATIONS
    },
