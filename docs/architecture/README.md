...
| Bounded Preview Lifecycle / current MCP surface `0.11` | `docs/superpowers/specs/2026-08-18-kodegpt-bounded-preview-lifecycle-design.md`, `docs/superpowers/plans/2026-08-18-kodegpt-bounded-preview-lifecycle.md`, and current `@kodegpt/core` + MCP/start-stack source/tests; composes existing background process authority into exactly `preview.start`, `preview.inspect`, and `preview.stop`, binds previews to fixed `127.0.0.1` HTTP endpoints with explicit caller-supplied ports, uses bounded HEAD readiness probing, adds no Rust RPC/dependency/persistent supervisor, and keeps arbitrary URL/host/browser/desktop/generic HTTP authority absent; aggregate public registry is exactly 65 tools |
...
- Provider Gateway core/operator authority is implemented privately and merged through PR #16 at canonical baseline `105547db2f1a8f97dc5ad6fb1a1efc1a12755607`, with readiness evidence at `docs/release/2026-08-16-provider-gateway-readiness.md`. `github.read.v1` supplies the five fixed read semantics exposed by PR #21. PR #23 adds a separately admitted `github.write.v1` with exactly `github.pr.create` and guarded `github.pr.merge`; mutation mappings are single-attempt only, merge requires exact `expectedHeadOid`, and ambiguous post-dispatch outcomes are reconciled rather than retried. Current source runtime/protocol/surface are `0.1 / 2026-07-28 / 0.11` with exactly 65 public tools and exactly seven `github.*` tools; the three additional tools are the bounded `preview.*` lifecycle and the CI mutations remain under the typed `ci.*` Remote-CI authority. Preview readiness is fixed to `127.0.0.1` and does not expose caller-selected host/URL/scheme, while the private Provider Gateway remains unchanged. No public `provider.*` tool, generic provider invocation, caller-selected provider endpoint/method/header/credential, or provider envelope is exposed. Live installed-release state is recorded separately in the execution tracker and may temporarily lag source during staged cutover.
