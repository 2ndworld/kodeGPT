...
| Trusted multi-toolchain + bounded impact + CI mutation / current MCP surface `0.10` | `docs/superpowers/specs/2026-08-17-kodegpt-trusted-toolchain-impact-ci-mutation-design.md`, `docs/superpowers/plans/2026-08-17-kodegpt-four-priority-followthrough.md`, current sandbox/Repository Intelligence/Remote-CI/MCP source and tests; current source keeps trusted Node/Rust composition inside validated Bubblewrap mounts, adds read-only `code.impact`, and adds typed `ci.rerun`, `ci.cancel`, and `ci.dispatch`; public registry is exactly 62 tools, mutation request budgets are one provider request, ambiguous mutation outcomes are not automatically retried, and generic provider/HTTP authority remains absent |
...
- Provider Gateway core/operator authority is implemented privately and merged through PR #16 at canonical baseline `105547db2f1a8f97dc5ad6fb1a1efc1a12755607`, with readiness evidence at `docs/release/2026-08-16-provider-gateway-readiness.md`. `github.read.v1` supplies the five fixed read semantics exposed by PR #21. PR #23 adds a separately admitted `github.write.v1` with exactly `github.pr.create` and guarded `github.pr.merge`; mutation mappings are single-attempt only, merge requires exact `expectedHeadOid`, and ambiguous post-dispatch outcomes are reconciled rather than retried. Current canonical source runtime/protocol/surface are `0.1 / 2026-07-28 / 0.10` with exactly 62 public tools and exactly seven `github.*` tools; the additional CI mutations remain under the typed `ci.*` Remote-CI authority and do not widen the private Provider Gateway. No public `provider.*` tool, generic provider invocation, caller-selected endpoint/method/header/credential, or provider envelope is exposed. Live installed-release state is recorded separately in the execution tracker and may temporarily lag canonical source during staged cutover.
