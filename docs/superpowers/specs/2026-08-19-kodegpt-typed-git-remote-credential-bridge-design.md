...
8. The authenticated Git command consumes that private config at command scope through fixed `-c include.path=/run/kodegpt/git-auth.config`. The config contains the exact-URL Authorization header plus explicit `followRedirects=false`, empty proxy, and `sslVerify=true`; CA discovery remains platform-native through Git/libcurl rather than pinning a distro-specific bundle path. `credential.helper=` remains disabled and `GIT_TERMINAL_PROMPT=0` remains enforced.
