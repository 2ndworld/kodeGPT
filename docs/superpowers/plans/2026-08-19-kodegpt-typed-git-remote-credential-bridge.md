...
- [ ] **Step 5: Implement ephemeral auth config** using fixed `x-access-token`, `base64`, exact-URL HTTP auth, redirect denial, empty proxy, and `sslVerify=true`; retain prompt/helper disabling while leaving CA discovery to platform Git/libcurl rather than pinning a distro-specific bundle path.
