...
Status: local implementation, exact source-head candidate verification through `a938b89614ff288de6c2a51129b274778f062ce9`, real systemd migration/upgrade/stop-start acceptance, installed-path provenance proof, and bounded ChatGPT smoke PASS. Final documentation-head verification, push/CI, final diff review, and PR #7 update/readiness remain gated below.
...
Remote PR review then found a fail-closed service-manager gap. Exact source head `a938b89614ff288de6c2a51129b274778f062ce9` fixed it with RED adapter and uninstall-preservation regression coverage and repeated the complete candidate command gate successfully: frozen install, Rust format, typecheck, build, full TypeScript suite at 82 files / 438 tests, sandbox functional tests, complete Rust workspace, protocol 11/11, integration 33/33, security 39/39, isolation 3/3, acceptance 5/5, forbidden scan, clean-prefix package smoke, and record-only benchmark. The benchmark recorded that exact Git head, and the passive Pranikah before/after comparison again returned `guard unchanged`.
...
Acceptance/final-gate verification found six concrete lifecycle defects and fixed each with regression coverage before continuing:
...
6. Remote PR patch review found that the systemd adapter treated every non-zero `stop` or `disable` exit as success. A user-manager/bus failure could therefore let `service stop` falsely report success or let `service uninstall` delete the unit and service-owned release metadata without proving the service had stopped. Exact source head `a938b89` makes `stop` and `disable` strict mutations while preserving the intentionally tolerant semantics of observational/idempotent helpers such as `show` and `reset-failed`. A RED adapter regression proves non-zero stop/disable results reject, and a command-level regression proves uninstall preserves the unit, release tree, and metadata when stop fails.
...
After defect 6 was committed as `a938b89`, a fourth real staged cutover proved strict service-manager mutations do not disturb the normal path: `rel_14a8916656bbe4045c0b52557fd8e975` remained active and listener-ready while candidate `rel_811e9083cea7418ece04285e60b3df60` was only staged; explicit `service restart` then promoted `rel_811e9083cea7418ece04285e60b3df60` and retained `rel_14a8916656bbe4045c0b52557fd8e975` as rollback. The resulting Node/Rust/zrok tree and service CWD resolve under the immutable installed release root. A real strict `service stop` then returned success with `state=stopped` and `listenerReady=false`; `service start` restored `state=running` and `listenerReady=true`. zrok still targets loopback port 43121 with reserved name `public:kodegpt-dev`, and no repository/worktree path is required by the running service.
...
Local runtime acceptance is complete on source head `a938b89`. Before PR #7 is considered final-ready, the remaining gates are limited to exact-documentation identity and remote integration readiness:
...
7. review the final remote diff/CI evidence and update PR #7 to the exact final head only if every gate remains green.
