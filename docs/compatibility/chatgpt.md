...
Development Efficiency v2 established semantic surface `0.16` with exactly 75 public tools; Developer Environment + Workspace Continuity retained 75 tools at `0.17`; Capability Intelligence Discovery established `0.18 / 76`; Semantic Repository Intelligence established `0.19 / 76`; and Evidence Freshness + Source-State Binding is the currently installed `0.20 / 76` service. The current ChatGPT connection has directly observed `system.capabilities` at `runtime 0.1 / protocol 2026-07-28 / surface 0.20 / 76 tools`, `system.health` healthy, `system.discover`, `context.build.focus`, and the source-state-bound evidence contracts. Continuity v2 + Resume Intelligence is the additive `0.21 / 76` source candidate: it keeps the public checkpoint schema at v1, privately persists bounded continuity schema v2 with server-captured source state and at most eight milestone snapshots, and adds `context.build(intent:"resume")` plus additive `workspace.info.continuity`/resume result metadata. It does not add a public tool, session/task database, scheduler, workflow engine, process/preview inventory API, autonomous agent runtime, or Codex dependency. Until `0.21` is merged, installed, and directly dogfooded through the target host, `0.20 / 76` remains the authoritative installed host evidence. Public `code.search(mode:"text"|"path")` remains exact/lexical; symbol/definition/reference search may report `precision:"structural"` for parser-backed TS/JS and remains truthful heuristic fallback where parser-backed support is unavailable. Historical host evidence for `0.19`, `0.18`, `0.17`, `0.16`, and older surfaces remains historical.

Continuity v2 + Resume Intelligence is the additive `0.21 / 76` candidate. Public `WorkspaceCheckpoint` remains schema v1, while private persistence uses a bounded schema-v2 envelope with server-captured source state and up to eight displaced checkpoint milestones. `context.build(intent="resume")` returns the normal bounded repository context plus deterministic resume synthesis: `fresh`, `stale`, `superseded`, or `unverifiable`. It follows only evidence refs already named by the current checkpoint, performs at most two Git ancestry reads for checkpoint relation, uses one-shot process/preview/PR/CI/artifact reads, and never polls, retries, restarts, cancels, reruns, dispatches, or invents a task/session database. Host orchestration remains in ChatGPT; no Codex execution dependency is introduced.
