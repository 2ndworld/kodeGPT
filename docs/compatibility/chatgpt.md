...
Development Efficiency v2 established semantic surface `0.16` with exactly 75 public tools; Developer Environment + Workspace Continuity retained 75 tools at `0.17`; Capability Intelligence Discovery established `0.18 / 76`; Semantic Repository Intelligence established `0.19 / 76`; Evidence Freshness + Source-State Binding established `0.20 / 76`; and Continuity v2 + Resume Intelligence is the currently installed `0.21 / 76` service. The current ChatGPT connection has directly observed `system.capabilities` at `runtime 0.1 / protocol 2026-07-28 / surface 0.21 / 76 tools`, healthy `system.health`, and resume-oriented `system.discover`; exact `context.build(intent:"resume")` host invocation still requires a refreshed host action snapshot when the current chat retains the older input enum. Bounded CI Long-Polling is the additive `0.22 / 76` source candidate: it adds only optional `ci.status.waitMs` while retaining the same eight CI tools, runtime/protocol, and host-owned orchestration. Public `code.search(mode:"text"|"path")` remains exact/lexical; symbol/definition/reference search may report `precision:"structural"` for parser-backed TS/JS and remains truthful heuristic fallback where parser-backed support is unavailable. Historical host evidence for `0.20`, `0.19`, `0.18`, `0.17`, `0.16`, and older surfaces remains historical.

Continuity v2 + Resume Intelligence is the installed `0.21 / 76` baseline. Public `WorkspaceCheckpoint` remains schema v1, while private persistence uses a bounded schema-v2 envelope with server-captured source state and up to eight displaced checkpoint milestones. `context.build(intent="resume")` returns the normal bounded repository context plus deterministic resume synthesis: `fresh`, `stale`, `superseded`, or `unverifiable`. It follows only evidence refs already named by the current checkpoint, performs at most two Git ancestry reads for checkpoint relation, uses one-shot process/preview/PR/CI/artifact reads, and never retries, restarts, cancels, reruns, dispatches, or invents a task/session database. Host orchestration remains in ChatGPT; no Codex execution dependency is introduced.

Bounded CI Long-Polling targets `0.22 / 76` by extending only `ci.status` with optional integer `waitMs` in `0..30000`. Revision resolution, repository identity, credential, and adapter are fixed once per logical call. Terminal `PASS`, `FAIL`, or `CANCELLED` evidence returns immediately; `PENDING` or `UNKNOWN` may be re-observed at most three additional times, for four observations total spread across the requested window. Each observation retains the existing six-request provider budget. There is no background CI monitor, durable queue, indefinite poller, hidden retry loop, or automatic CI mutation.
...
- `workspace.inspect` assembles bounded repository evidence about project types, languages, manifests, entrypoints, areas, symbols, and relationships from retained-root inspection. For supported TS/JS files the `0.19` surface enriches that evidence with bounded compiler-AST declarations, references, relative module relationships, and source regions; unsupported languages remain truthful heuristic fallback. `context.build` composes existing inspect/Git/search/verification/read capabilities using deterministic rules and a byte budget; optional `focus` requires an explicit target path and derives the target region from the existing bounded target source read with the same structural parser, so large full-workspace aggregation limits do not erase the requested region and no file-root tree inspection is required. It performs no model inference.
...
- `git.changes` provides a content-sensitive deterministic checkpoint. On installed surface `0.20` it also returns `sourceState:{headOid,changesFingerprint}`, where `changesFingerprint` is exactly the top-level fingerprint and `headOid` comes from the same hardened Git status scan. Untracked content participates in fingerprint identity, while v1 unified patch coverage is explicitly limited to staged and worktree tracked diffs.
- `verify.list` discovers only named deterministic recipes. With an optional target it selects the nearest project ecosystem, prioritizes target package/crate recipes, and keeps root/full-gate recipes as fallback. `verify.run` selects and re-resolves one of those recipes through the existing process sandbox; on installed surface `0.20` it captures source state immediately before process launch and returns that immutable `sourceState` with the operation. It does not accept an arbitrary replacement executable/argv or shell command.
- Preview creation on installed surface `0.20` captures source state once before launch and retains that immutable state across `preview.start` / `inspect` / `stop`; browser sessions inherit the preview state and propagate it through open/inspect/actions/screenshot/console/network-failure evidence, while visual matrix/compare results inherit the same state without independent Git scans.
...
- On installed surface `0.21`, `context.build(intent:"resume")` composes the normal bounded repository context with a deterministic continuity synthesis. Checkpoint relation is `fresh`, `stale`, `superseded`, or `unverifiable`; current source state comes from the base `context.build` Git evidence rather than a second Git scan. Only evidence refs already named by the checkpoint are followed, each through bounded read-only authority, and failures degrade per ref rather than triggering retries or mutation.
- On the `0.22` candidate, `ci.status` keeps its current zero-wait fast path and adds optional `waitMs` bounded to 30 seconds. One logical call resolves the target revision once, observes CI at most four times, preserves the existing per-observation provider budget, exits immediately on terminal evidence, and never creates background monitoring or automatic retry/mutation authority.
