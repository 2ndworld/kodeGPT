...
Development Efficiency v2 established semantic surface `0.16` with exactly 75 public tools by removing `file.search`, `deploy.preview.create`, and `deploy.preview.inspect`; the later Developer Environment + Workspace Continuity baseline retained 75 tools at surface `0.17` by adding bounded CAS-backed `workspace.checkpoint` and removing the unused metadata-only `extension.list` one-for-one. Capability Intelligence Discovery established the historical `0.18 / 76` baseline. Semantic Repository Intelligence remains the currently installed release at `0.19 / 76` on immutable release `rel_f8ed46a490b83951f30a3bc6a01f4c0c`, with known-good `0.18` release `rel_fda9290d7ee09062dd6a656b56292683` retained as rollback. Evidence Freshness + Source-State Binding is the additive `0.20 / 76` source candidate: it adds `SourceStateRef { headOid, changesFingerprint }` to Git-change, verification, preview, browser, and visual evidence without changing runtime `0.1`, MCP protocol `2026-07-28`, tool names, or required inputs. Until that candidate is merged, installed, and the ChatGPT action snapshot is refreshed, existing `0.19` host observations remain the latest installed host evidence. After cutover, the current ChatGPT connection directly observed `system.capabilities` at `0.19 / 76` and `system.health` as healthy; exact-active-release MCP stdio dogfood additionally observed the optional `context.build.focus` schema, structural TS/JS search, and focused source-region output. The approved action definition cached in this conversation still lacks the new optional `focus` input, so this conversation is not evidence that the ChatGPT action schema itself has refreshed to `0.19`; a fresh/rescanned action snapshot is still required before claiming host-visible `context.build.focus`. Public `code.search(mode:"text"|"path")` remains exact/lexical; symbol/definition/reference search may report `precision:"structural"` for parser-backed TS/JS and remains truthful heuristic fallback where parser-backed support is unavailable. Historical host evidence for `0.18`, `0.17`, `0.16`, and older surfaces remains historical.
