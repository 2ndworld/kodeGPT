...
Development Efficiency v2 established semantic surface `0.16` with exactly 75 public tools by removing `file.search`, `deploy.preview.create`, and `deploy.preview.inspect`; the later Developer Environment + Workspace Continuity baseline retained 75 tools at surface `0.17` by adding bounded CAS-backed `workspace.checkpoint` and removing the unused metadata-only `extension.list` one-for-one. Capability Intelligence Discovery is now the live KodeGPT release at semantic surface `0.18` with exactly 76 public tools, adding only read-only `system.discover` plus deterministic optional `skill.list(query)` ranking and additive `skill.inspect.requirementGraph` core/stage evidence while preserving Agent Skills as data/guidance rather than an executable plugin, session, workflow, or agent runtime. The installed service is active on `rel_fda9290d7ee09062dd6a656b56292683`, and the existing ChatGPT connection has observed live `system.capabilities` reporting `0.18 / 76` plus discovery feature flags. However, this already-open conversation still retains the previously approved 75-action host snapshot: the host-visible action list does not yet contain `system.discover`, and its generated `skill.list` schema does not yet include `query`. Exact-active-release stdio dogfood verifies both new public contracts at the backend, but refreshed-host invocation remains unclaimed until the ChatGPT connector actions are rescanned/refreshed. Public lexical repository search remains available through `code.search(mode:"text")`, backed by retained private search authority. Historical host evidence for `0.17`, `0.16`, and older surfaces remains historical and must not be treated as refreshed `0.18` action evidence.
