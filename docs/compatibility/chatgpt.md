...
Development Efficiency v2 established semantic surface `0.16` with exactly 75 public tools by removing `file.search`, `deploy.preview.create`, and `deploy.preview.inspect`; the later Developer Environment + Workspace Continuity baseline retained 75 tools at surface `0.17` by adding bounded CAS-backed `workspace.checkpoint` and removing the unused metadata-only `extension.list` one-for-one. Capability Intelligence Discovery is now the live KodeGPT release at semantic surface `0.18` with exactly 76 public tools, adding only read-only `system.discover` plus deterministic optional `skill.list(query)` ranking and additive `skill.inspect.requirementGraph` core/stage evidence while preserving Agent Skills as data/guidance rather than an executable plugin, session, workflow, or agent runtime. The installed service is active on `rel_fda9290d7ee09062dd6a656b56292683`. A fresh ChatGPT conversation with the refreshed action snapshot directly observed live `system.capabilities` reporting `0.18 / 76` plus discovery feature flags, invoked `system.discover({query:"cek tampilan mobile"})` and the workspace-scoped variant with `visual.captureMatrix` ranked first, invoked `skill.list({workspaceId,query:"application development workflow",limit:20})`, and observed `skill.inspect.requirementGraph.schemaVersion=1` with core `NATIVE` plus the declared browser, CI, continuity, Git-delivery, implementation, preview, pull-request, repository-understanding, verification, and visual stages. Exact-active-release stdio dogfood remains an independent backend-level confirmation of the same contracts. Public lexical repository search remains available through `code.search(mode:"text")`, backed by retained private search authority. Historical host evidence for `0.17`, `0.16`, and older surfaces remains historical and must not be treated as current `0.18` action evidence.
