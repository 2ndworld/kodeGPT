...
The Public Surface Reduction candidate advances the semantic surface to `0.16` with exactly 75 public tools by removing `file.search`, `deploy.preview.create`, and `deploy.preview.inspect`. Public lexical repository search remains available through `code.search(mode:"text")`, backed by the retained private search authority, while provider-specific Netlify deployment is no longer an MCP action. Historical host evidence for older surfaces remains historical and must not be treated as `0.16` evidence.
